...
### ADR-097: 12단계 EAS iOS 빌드 정합 — RN 생태계 SDK 고정 + static framework

- 결정: RN 생태계를 Expo SDK 55가 고정한 버전으로 정렬(드리프트한 0.85.x에서 **다운그레이드**): `react-native@0.83.6`, `react`·`react-dom@19.2.0`, `react-native-safe-area-context@~5.6.2`, `react-native-screens@~4.23.0`, `react-native-svg@15.15.3`, `react-native-webview@13.16.0`. 추가로 `@react-native-google-signin/google-signin@16`의 `AppCheckCore` 정적 통합을 위해 `expo-build-properties`로 `ios.useFrameworks="static"`.
- 이유: Expo SDK는 **단일 RN 버전에 맞춰 빌드·테스트된 묶음** — `babel-preset-expo@55.0.22`가 `@react-native/babel-preset`을 **정확히 0.83.6으로 고정** 의존. RN만 0.85.3이면 코드젠 babel 플러그인이 RN 0.85의 `VirtualViewNativeComponent`(`onModeChange`)를 파싱 실패(`expo export:embed --eager`=production 번들에서만 터지고, dev Metro는 지연 변환이라 우회돼 안 보임). precompiled 프레임워크·expo 네이티브 모듈도 RN 0.83.6 ABI로 컴파일돼 있어 RN 상향은 네이티브(Fabric/TurboModule) **런타임 불일치** 위험 → SDK 정렬이 유일 안전책. 대안 기각: SDK 업그레이드=범위 과대·breaking 위험(배포 중 부적합), babel-preset만 상향=네이티브 ABI 불일치 미해결. `AppCheckCore`(→`GoogleUtilities`/`RecaptchaInterop`)는 모듈 미정의라 정적 라이브러리 통합 불가 → static framework로 module map 생성.
- 검증: `tsc --noEmit`·`eslint`·`expo export`(Hermes 번들) 모두 통과. 미커밋 상태라 `git checkout apps/mobile/{package.json,app.config.ts} pnpm-lock.yaml` + `pnpm install`로 즉시 롤백 가능. 최종 검증은 EAS 빌드 + 실기기.
