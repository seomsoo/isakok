...
> 마지막 업데이트: 2026-06-05 (10-4 코드 머지 PR #61 + 배포 진행: db push 00021/00022·Edge Function 4종·Apple revoke 시크릿 완료(MCP/사용자 확인). 별건: WebView 콜드로드 견고화 — 실기기 검증 완료·커밋 대기. 남은 건: Kakao 콘솔·App Store Connect 등 → "다음 할 것" 참고)
...
배포 진행(2026-06-05 확인): ① `supabase db push`(00021/00022) **✅** → ② Edge Function 배포(apple-token-exchange·delete-account·cleanup·kakao-unlink-webhook) **✅** → ③ Apple revoke 시크릿(.p8/TEAM/KEY/CLIENT) **✅**(사용자 확인) → ④ Kakao 콘솔 콜백 URL + 위조 user*id smoke·나머지 시크릿(KAKAO*\*/CLEANUP_TOKEN/DRY_RUN) **확인 필요** → ⑤ RLS CI required check **확인 필요** → ⑥ App Store Connect·EAS production·TestFlight·Data Safety **확인 필요**.
...
- 10-4 **코드 머지 완료**(PR #61). 현재 **배포·콘솔 운영 단계** — 상세는 아래 "다음 할 것".
- WebView 콜드 로드 견고화(자동 재시도 + 스톨 타임아웃 + RNW 기본 에러 억제) — 코드 완료, 실기기(USB) 검증 완료, **커밋 대기**(브랜치 `fix/webview-cold-load`). 결정·Stage 2/3 계획은 **ADR-084** 참조 (Stage 2/3는 출시 후 평가로 보류)
- 문서 구조 정리(이번 세션) — STATUS 슬림화(완료 인덱스화)·ADR 분리(`docs/ADR.md`, ADR-001~084)·하위 CLAUDE.md 6종 현행화·handoff 규칙 개정·.gitkeep/잔여 worktree 정리. **커밋 대기**(브랜치 `docs/restructure-docs`)
- (10-3은 main 머지 완료 — PR #59 `2da4380`, DB 백업 pg_dump 수정 #60)

## 다음 할 것

> 배포 상태 (2026-06-05 MCP 확인): `db push` 00021/00022 ✅ · Edge Function 배포(apple-token-exchange·delete-account·cleanup·kakao-unlink-webhook) ✅ · Apple revoke 시크릿 ✅(사용자 확인) · dev-wipe.sql 삭제 ✅. 아래는 남은·미확인 항목.

1. **커밋/PR — 2 브랜치** (현재 미커밋, main 기준 분기):
   - `fix/webview-cold-load` ← `apps/mobile/src/components/WebViewScreen.tsx` (Codex P1/P2 반영 완료)
   - `docs/restructure-docs` ← STATUS·`ADR.md`·DECISIONS·UI-POLISH·CLAUDE.md(루트+하위 5)·handoff.md·.gitkeep 정리
2. **콘솔·운영 확인 필요** (MCP로 확인 불가 → 직접 점검):
   - Kakao 콘솔 콜백 URL 등록 + 위조 user_id smoke
   - 나머지 시크릿: CLEANUP_TOKEN, DRY_RUN, KAKAO_APP_ID, KAKAO_ADMIN_KEY
   - cron-setup.sql 적용 (cleanup 스케줄)
   - 브랜치 보호 RLS CI required check 등록
   - App Store Connect · EAS production · TestFlight · Data Safety(App Privacy)
