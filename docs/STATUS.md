...
- **마이크로 인터랙션 정합 패스** (이 세션, **미커밋**) — 햅틱(`requestHaptic`)·등장 페이드·바텀시트 진입·Toast 퇴장·체크 pop·press/duration 100ms 정합. `pnpm lint`/`typecheck`/`build` + 온보딩 스모크(Playwright, 햅틱 dev-fallback) 통과. 상세 `UI-POLISH.md` §13. **다음: 작업단위 커밋 + 실기기 햅틱 체감·데이터 의존 화면 시각확인.**
