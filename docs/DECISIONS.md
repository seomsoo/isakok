...
> ⚠️ **이 문서는 프로젝트 기획 시점의 초기 설계 스냅샷**(§1~13)입니다. 작성 후 대부분 미갱신 — 이후의 변경·구체화·정본 의사결정은 **`docs/ADR.md`(ADR-001~)** 를 참조하세요. 일부 항목은 ADR으로 갱신/대체되었습니다(예: 인증 ADR-041~054, dev=prod ADR-075, WebView ADR-035/036/084).
...
> (이 로드맵은 기획 시점 기준 — 현황·다음 작업은 `docs/STATUS.md` 참조)
...
→ ADR-001~084는 **`docs/ADR.md`**로 이관(2026-06-05). 번호로 인용된 `ADR-NNN` 참조는 파일 위치와 무관하게 그대로 유효.
