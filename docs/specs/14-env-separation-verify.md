...
- [x] **9. 백업** — `SUPABASE_PROD_DB_URL` 시크릿 생성 + 워크플로 참조 전환 + 헤더 각인 [x]. 나머지는 default branch 제약으로 머지 후 이연이었고 **머지 직후 전부 완료**: 구 `SUPABASE_DB_URL` 삭제 → dispatch 그린(2회 실패 후 — ① `!` 러너의 대화형 프롬프트 무산으로 빈 시크릿 저장 ② 클립보드에 비번만 복사되어 URL 아님, pooler 전체 URL로 재설정 후 성공) → **아티팩트 restore 테스트 통과**(일회용 로컬 PG14에 복원 — public 테이블 11개, master 46·users 35·moves 12·checklist 485·ai_cache 5·push_log 124 sanity, 예상 외 에러 0, 덤프·클러스터 즉시 삭제). PG17 덤프↔PG14 복원의 auth 스키마 부재·설정 파라미터 에러는 vanilla PG 예상 잡음으로 분류.
...
머지 후 잔여 처리 결과(PR #86 머지 직후): 구 시크릿 삭제 ① ✅ · 백업 dispatch+restore ② ✅(§14-9 갱신 참조) · `main:dev` 재발행 ③ ✅(dev 웹 번들에 DEV 배지·fingerprint 반영 실측) · `SB_MGMT_TOKEN` 제거 ④ ✅ · 키 rotate ⑤ = 사용자 판단 잔여(**prod DB 비밀번호가 백업 재설정 과정에서 세션 대화에 노출 — rotate 권장 등재**). 별개 잔여: §14-6 회원 전용 영역(linkIdentity 승격·사진 게이트·계정 삭제) dev 완주 실측.
