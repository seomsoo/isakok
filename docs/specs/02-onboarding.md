...
│  └─────────────────────┘    │
│                             │
│  이미 시작한 이사가 있나요?   │  ← 로그인 링크 (10단계에서 연동)
│  로그인                      │
└─────────────────────────────┘
```
...
- "로그인" 링크 → 10단계에서 로그인 모달로 연결. 2단계에서는 `console.log('TODO: 로그인')` + 비활성 스타일
...
- 10단계에서 RLS 켜면: 비회원은 IndexedDB 로컬 동작으로 전환
