...
└── agents/
    ...
    ├── native-a11y-reviewer.md   ← 생성 (9단계 Expo 셸부터 활성)
...
| native-a11y-reviewer | 9단계(Expo 셸)부터 활성                                       | Expo 셸 도입 시 활성 |
...
- E(인증)은 10단계 도입 후 활성. 8-2 단계에선 RLS 정책 의미만 검토 (RLS는 8-1에서 정의는 됐으나 enable 안 됨)
...
- 9단계(Expo 셸)부터 활성. 미리 만들어두면 진입 시 의식하기 쉬움
...
- (B) 분리 (채택) — 도메인별 깊이 확보. native는 9단계(Expo 셸)부터 활성
...
위 체크리스트의 모든 항목이 ✅이고 STATUS.md에 다음이 기록되면 9단계(Expo 셸) 시작:
...
- Expo(React Native) 네이티브 셸 생성 + WebView 래핑
- 네이티브 하단 탭바, 스플래시 스크린, 브릿지 프로토콜
- native-a11y-reviewer 활성화
- apps/web Vercel 배포
...
8-2 dry-run → apply 전환은 이후 단계와 독립적으로 운영 판단. MVP 출시 전후 안정성 확보 후 결정.
